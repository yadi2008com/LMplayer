...
public class LrcShowFragment extends Fragment{
	    ...
	    public static final String TTpath = "mnt/sdcard/TMusic/";//音乐文件存放目录
	    View lrc_view= null;
        TextView tv_nolrc;
	    ...
	    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
	    	...
	    	tv_nolrc = (TextView)lrc_view.findViewById(R.id.tv_nolrc);
	     	...
	     	intent = new Intent(ApolloService.UPDATE_LRC_ACTION);//监听事件UPDATE_LRC_ACTION
	 ...
	 // ------------------------------歌词显示-------------------------------------------------
		...
		private void ShowLyric(String path) {
			//"mnt/sdcard/external_sd/MP3/"+MusicUtils.getTrackName()+"."+"mp3",格式例子标识
...
//			lyricView.setLrcTextSize(musicPreference.getLrcSize(context));
		...
		// -------------------lrc //有歌词的地址，从网上下载歌词解析一遍--------------------------------------
		public static boolean isupdatalrc = true;
		class UIUpdateThread implements Runnable {
			long time = 1000;

			public void run() {
				while (isupdatalrc) {//动态更新歌词，工作线程
					try {
		...
		Handler nameshandler = new Handler() {
			...
			public void handleMessage(Message msg) {
				...
				case 10:
					//Log.i("music", "下载保存完毕-开始更新 再 显示到UI--------");
					ShowLyric(TTpath + MusicUtils.getTrackName()+ "-"
