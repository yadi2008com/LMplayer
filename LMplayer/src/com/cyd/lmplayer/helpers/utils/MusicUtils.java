...
package com.cyd.lmplayer.helpers.utils;
...
import com.cyd.lmplayer.service.ServiceBinder;
...
import static com.cyd.lmplayer.Constants.GENRES_DB;
import static com.cyd.lmplayer.Constants.PLAYLIST_NAME_FAVORITES;
import static com.cyd.lmplayer.Constants.PLAYLIST_NEW;
import static com.cyd.lmplayer.Constants.PLAYLIST_QUEUE;

/**
 * Various methods used to help with specific music statements
 * 帮助service和activity连接
 */
public class MusicUtils {
     ...
     * @return
     */
    public static long[] getQueue() {//获得当前播放队列
     ...
     * @return
     */
    public static long createPlaylist(Context context, String name) {//创建播放列表,加到本地数据库
    ...
    public static long getFavoritesId(Context context) {
        ...
        if (cursor.getCount() <= 0) {
            favorites_id = createPlaylist(context, "最喜爱的");
        } else {
            cursor.moveToFirst();
	 ...
	 * @param id
	 */
	public static void setRingtone(Context context,String id){//更改铃声文件
	...
	public static void RealdeletSong(Context context,long id,int postion){
		...
		Toast.makeText(context, "成功删除"+count+"个文件", Toast.LENGTH_SHORT).show();
